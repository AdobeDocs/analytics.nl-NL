...
| **`clickmaplink`** | De koppeling Activity Map | varchar(255) |
...
| **`dataprivacyconsentoptout`** | Variabele gebruikt in de [Weigering van toegang voor beheer](/help/components/dimensions/cm-opt-out.md) dimensie. Meerdere waarden kunnen aanwezig zijn per treffer, gescheiden door een pijp (`\|`). Geldige waarden zijn `SSF`, `DMP`en `SELL`. | varchar(100) |
...
| **`event_list`** | Door komma&#39;s gescheiden lijst met numerieke id&#39;s die de gebeurtenissen vertegenwoordigen die tijdens de hit zijn geactiveerd. Deze groep bevat zowel standaardgebeurtenissen als aangepaste gebeurtenissen 1-1000. Gebruikt `event.tsv` opzoeken. | Tekst |
...
| **`hier1 - hier5`** | Wordt gebruikt door hiërarchievariabelen. Bevat een lijst met gescheiden waarden. Het scheidingsteken wordt geselecteerd onder Rapportsuite-instellingen. | varchar(255) |
...
| **`ip2`** | Niet gebruikt. Variabele van de back-endverwijzing voor rapportsuites met VISTA-regels op basis van IP-adres. | char (20) |
| **`ipv6`** | Het gecomprimeerde IPv6-adres, indien beschikbaar. Uitsluiten van `ip`; is leeg als deze kolom een niet-verhuld IP-adres `ip` bevat. | varchar(40) |
...
| **`mobileappperformanceappid`** | Wordt gebruikt in de Apteligent-gegevensconnector. De app-ID die wordt gebruikt in Apteligent. | varchar(255) |
...
| **`mobilecampaignname`** | Naam van de campagne, ook opgeslagen in de campagnevariabele. Ingevuld door mobiele app-aankoop. | Varchar (255) |
...
| **`mobilemessagepushoptin`** | Verzameld op basis van de variabele contextgegevens `a.push.optin`. Stel deze optie in op &#39;true&#39; wanneer de gebruiker ervoor kiest om pushberichten te plaatsen; anders is de waarde &quot;false&quot;. | varchar(255) |
...
| **`mobilerelaunchcampaigntrackingcode`** | Verzameld uit de variabele `a.launch.campaign.trackingcode`van de contextgegevens. Wordt gebruikt in aankopen als de code voor het bijhouden van de opstartiecampagne. | varchar(255) |
...
| **`page_type`** | Wordt gebruikt om de [dimensie Pagina&#39;s in te vullen die niet zijn gevonden](/help/components/dimensions/pages-not-found.md) . Wordt uitsluitend gebruikt voor 404 pagina&#39;s. Deze variabele moet leeg zijn of de waarde `ErrorPage`bevatten. | char (20) |
| **`page_url`** | De URL van de hit. Opmerking: deze `post_page_url` optie is gestript voor aanvragen voor het bijhouden van koppelingen en gebruikt het gegevenstype varchar(255). | text |
...
| **`resolution`** | Numerieke id die de resolutie van de monitor vertegenwoordigt. Gebruikt in de [Monitorresolutie](/help/components/dimensions/monitor-resolution.md) dimensie. Gebruikt `resolution.tsv` een opzoektabel. | smallint niet ondertekend |
| **`s_kwcid`** | Trefwoord-id die wordt gebruikt in Adobe Advertising-integraties. | Varchar (255) |
| **`s_resolution`** | Waarde voor raw-schermresolutie. Verzameld met de JavaScript-functie `screen.width x screen.height`. | teken(20) |
| **`search_engine`** | Numerieke ID die de zoekmachine vertegenwoordigt die de bezoeker naar uw site heeft doorverwezen. Gebruikt `search_engines.tsv` opzoeken. | smallint niet ondertekend |
| **`search_page_num`** | Deze optie wordt gebruikt door de [dimensie Alle zoekposities](/help/components/dimensions/all-search-page-rank.md) . Geeft aan op welke pagina met zoekresultaten uw site werd weergegeven voordat de gebruiker op uw site klikte. | small int zonder teken |
...
| **`sourceid`** | Bron-id | niet ondertekend |
| **`state`** | Statusvariabele. | Varchar(50) |
| **`stats_server`** | Niet van gebruik. Adobe interne server die de hit heeft verwerkt. | teken(30) |
| **`survey`** | Niet meer gebruikt. Variabele van het Adobe Survey-onderzoek. Alleen de `post` kolom is beschikbaar. | Tekst |
| **`survey_instances`** | Niet meer gebruikt. Variabele van adobe Survey-instanties. | text |
...
| **`tnt`** | Gebruikt in Adobe Target-integraties. Dit zijn alle tests waarvoor ze momenteel in aanmerking komen. De indeling is: `TargetCampaignID:TargetRecipeID:TargetType\|Event/Action`. | text |
...
| **`user_hash`** | Niet gebruiken. Hash op de rapportsuite-id. Gebruiken `username` in plaats daarvan. | niet ondertekend |
