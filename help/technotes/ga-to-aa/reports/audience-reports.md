...
Deze pagina gaat ervan uit dat de gebruiker een basiskennis heeft van het gebruik van Analysis Workspace. Zie [Een basisrapport maken in Analysis Workspace voor gebruikers van Googles Analytics](create-report.md) als u het gereedschap nog niet kent in Adobe Analytics.

## Actieve gebruikers

Actieve gebruikers laten het totale aantal gebruikers van uw site zien in de voorafgaande 1, 7, 14 of 28 dagen. Hoewel de Adobe niet de nauwkeurige berekening heeft die in Googles Analytics wordt gebruikt, kunt u de metrische Unieke Bezoekers gebruiken om een gededupliceerde telling van gebruikers aan uw plaats te zien die op de geselecteerde datumwaaier wordt gebaseerd.
...
De waarde van het leven is een eigenschap die extra gespecialiseerde implementatie op beide platforms vereist. Adobe beveelt aan om samen met een implementatieconsultant deze gegevens te verkrijgen.
...
Het verslag van het publiek in Googles Analytics vereist de instelling van een publiek. De soorten publiek vereisen ook configuratie in Adobe door Adobe Audience Manager. Raadpleeg de gebruikershandleiding van Adobe Audience Manager voor meer informatie.

## Gebruikersverkenner

Het rapport van de Ontdekkingsreiziger van de Gebruiker staat een analist toe om individuele bezoeken door anonymized herkenningstekens te bekijken. Adobe heeft geen invloed op de achtergrondidentifiers buiten gegevensfeeds, die onbewerkte gegevensexport op raakniveau zijn.
...
Demografische gegevens en gegevens over interesses bieden informatie over de leeftijd, het geslacht en de belangen van sitegebruikers. Deze gegevens worden door Google verzameld op basis van hun mogelijkheden voor intersite tracering.

Demografische gegevens en interesses worden niet automatisch door de Adobe verzameld. Als uw organisatie deze gegevens echter verkrijgt, kunt u klantkenmerken gebruiken, een functie in het Adobe Experience Cloud-platform. Het maakt volledige controle mogelijk over de organisatie van gegevens op basis van kenmerken, en is niet beperkt tot demografie of belangen.

Raadpleeg Customer attributes Help voor meer informatie.
...
1. Zoek in het optiemenu van de componenten de **Eerste bezoeken** en sleep het naar het grote vrije gebied van de vrije vormlijst geëtiketteerd &quot;Dropeer een Dimension hier&quot;. Let op: **Eerste bezoeken** is een segment, terwijl Workspace typisch dimensies gebruikt om rijen te vertegenwoordigen.
...
* De **Versie Flash** De primaire dimensie is niet beschikbaar in Adobe Analytics, maar deze gegevens kunnen desgewenst door een eVar worden verzameld.
...
2. Zoek de **Pagina&#39;s** en klikt u op het pictogram Pijl om de paginawaarden weer te geven. Dimensionen zijn geel.
3. Zoek de gewenste paginawaarde waarmee u wilt beginnen en sleep deze naar de ruimte met het label Dimension of item in het midden
