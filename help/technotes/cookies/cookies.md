...
De bezoeker-id-service gebruikt het cookie [`demdex.net`](https://experienceleague.adobe.com/docs/id-service/using/intro/cookies.html) om bezoekers een permanente id in verschillende klantdomeinen te geven. In browsers waar cookies van derden worden geblokkeerd, is interdomeintracering niet beschikbaar.

### First-party koekjesbeperkingen {#limitations-first-party-cookies}

Cookies van eerste bedrijven zijn toegestaan in alle grote browsers. Apple beperkt echter de levensduur van cookies van eerste bedrijven die door Adobe zijn ingesteld via het Intelligent Tracking Program (ITP). Dit geldt ook voor Safari op Mac OS en voor alle browsers op iOS en iPadOS.

Cookies van eerste partijen zijn beperkt tot een vervaldatum van 7 dagen of een vervaldatum van 24 uur voor doorklikken die Apple bepaalt van trackers. Als een gebruiker bij een vervaldatum van 7 dagen uw site bezoekt en vervolgens binnen die zeven dagen terugkeert, wordt de vervaldatum van de cookie met nog eens 7 dagen verlengd. Als een gebruiker echter uw site bezoekt en vervolgens binnen acht dagen terugkeert, wordt hij of zij tijdens het tweede bezoek behandeld als een nieuwe gebruiker.
...
#### Tijdlijn van belangrijke wijzigingen in ITP-beleid
...
Het beleid van ITP evolueert vaak. Voor het recentste beleid, zie [Preventie van het Volgen in Webkit](https://webkit.org/tracking-prevention).
...
* [`AMCV` ](https://experienceleague.adobe.com/docs/id-service/using/intro/cookies.html) cookies door de Adobe Experience Cloud Visitor ID (ECID) service library
...
* De Analytics-verouderde [`s_fid` cookie](https://experienceleague.adobe.com/docs/core-services/interface/ec-cookies/cookies-analytics.html). Dit is het fallback-cookie die wordt gebruikt wanneer `s_vi` niet kan worden ingesteld

#### Wat is de impact van ITP op Safari voor Analytics?

De impact van de ITP-beperkingen zal aanzienlijk variëren, afhankelijk van het gedrag van uw gebruikers. Alleen bezoekers die een browser gebruiken die invloed heeft op ITP (Safari) en die terugkeren na een afwezigheid van zeven dagen, worden hierdoor beïnvloed. Als bezoekers niet binnen zeven dagen een ITP-browser gebruiken of terugkeren, hebben ze geen invloed. Het is belangrijk om uw eigen gegevens in Analytics te herzien om de omvang van de gevolgen van deze beperking te begrijpen. Voor uiteinden op hoe te om het effect aan uw plaatsen te meten, zie &quot;[Hoe kan ik bepalen als de veranderingen van Safari mijn zaken beïnvloeden?](#measure-itp-effect)&quot;
...
2. Wijzigingen in kenmerk. Conversiegebeurtenissen zijn gekoppeld aan voorafgaande activiteiten van dezelfde bezoeker. Wanneer een cookie is verlopen, worden toekomstige gebeurtenissen gekoppeld aan een nieuwe bezoeker en kunnen conversies niet meer worden gekoppeld aan de oorspronkelijke bezoeker.
...
Met de release van Chrome 80 browser in februari 2020 — en opeenvolgende versies van Firefox- en Edge-browsers — dwingt het SameSite cookie-kenmerk de specificatie voor drie verschillende waarden af om het gedrag van aanvragen voor meerdere sites te beheren:
...
Alle Adobe cookie-updates worden verwerkt via Adobe-servers en Adobe Edge-servers stellen de juiste cookie-kenmerken in. Alle cookies van derden zijn op de server bijgewerkt met de juiste kenmerken. Er zijn geen JavaScript-updates vereist voor uw sites.

Deze upgrade door Adobe edge-servers werd automatisch uitgevoerd terwijl gebruikers een website bezochten waar de cookie werd gebruikt. Voor de meeste Adobe-producten hadden cookies de juiste vlaggen toen Chrome 80 in 2020 werd uitgebracht. De uitzondering was de implementaties van Adobe Analytics die derdegegevensinzameling gebruiken en de dienst van identiteitskaart van de Bezoeker van de Experience Cloud niet gebruiken. Voor dat type implementatie moet u de klantenservice vragen om de vlaggen te wijzigen; zie &quot;[Verander de waarde SameSite wanneer u één CNAME voor veelvoudige domeinen](#samesite-one-cname)&quot;in de volgende sectie voor meer informatie gebruikt. Totdat de vlag wordt gewijzigd, kunnen deze klanten een kleine, tijdelijke toename van nieuwe bezoekers ervaren die anders zouden worden geëtiketteerd als terugkerende bezoekers.
...
* Meet het percentage van uw verkeer van ITP-Beheerde browsers:
   ...
   2. Pas het segment op het aantal bezoeken toe om het relatieve gebruik van Safari in uw gebruikersbasis te begrijpen. Hiermee kunt u een tabel als volgt maken:
