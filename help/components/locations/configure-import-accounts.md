...
## Beginnen met het maken of bewerken van een account op de pagina Accounts
...
1. Ga met [ verder vormen een plaatsrekening ](#configure-a-location-account).

## Een locatieaccount configureren

Een cloudimport- of -exportaccount configureren nadat u deze hebt gemaakt of bewerkt:
...
1. In de [!UICONTROL **eigenschappen van de Rekening**] sectie, specificeer informatie specifiek voor het accounttype dat u selecteerde.
   ...
   **Verouderde rekeningstypes**
   ...
   SFTP-ondersteuning voor gegevensfeeds is beschikbaar. Dit vereist een gastheer SFTP, een gebruikersbenaming, en de bestemmingsplaats om een geldige RSA of DSA openbare sleutel te bevatten. U kunt de juiste openbare sleutel downloaden wanneer u de feed maakt.

   Wanneer het downloaden van RSA of DSA openbare sleutel voor gegevensvoer, doe één van beiden van het volgende:

   * Wijzig de naam van het bestand dat u naar het openbare-sleutelbestand hebt gedownload in `authorized_keys` en upload het bestand vervolgens naar de map `.ssh` op uw SFTP-server.

   * Als u een bestaand `authorized_keys` -bestand hebt dat al andere sleutels bevat, voegt u de sleutel van Adobe toe aan het bestaande `authorized_keys` -bestand. Zorg er daarbij voor dat u de bestaande toetsen niet overschrijft.
