...
1. Configureer de paginalay-out van het dashboard:
   ...
   * **[!UICONTROL Add Page]**: Voegt een lege pagina toe aan het dashboard, waarop u inhoud kunt slepen om rapporten te maken.
   * **[!UICONTROL Paper]**: Hiermee kunt u een papierformaat opgeven, zoals Liggend, Staand en A4.
...
1. (Optioneel) Klik op **[!UICONTROL More Options]** > **[!UICONTROL Set as Landing Page]**.
...
1. (Optioneel) Vorm het datumbereik.
   ...
   * **[!UICONTROL Fixed]**: Hiermee voorkomt u dat de datum in de tijd vooruit gaat.
...
1. Klikken **[!UICONTROL Add Content]** en sleep items naar het dashboard.
   ...
   De [!UICONTROL Add Content] wordt de rapportinhoud van andere dashboards, erfenis dashboards, en gedeelde dashboards weergegeven.
   ...
   * Het Samenvatting van het bedrijf: De paginameningen van vertoningen voor veelvoudige rapportreeksen en metriek die u selecteert.
   * Metrisch bereik: geeft een maat weer dat aangeeft waar de cijfers van de meetwaarden zich bevinden ten opzichte van de drempels die u opgeeft.

     U kunt metrisch, grafiektype, kleurenwaaier, en drempelwaarden selecteren. Als het aantal metrieke stijgt boven de drempel Groter dan, wijst het meetbereik dit in rapportlet aan, gebruikend de kleur boven het Groter dan gebied. Als het aantal metrische waarden lager is dan de drempelwaarde, geeft het meetbereik dit aan in het rapport, waarbij de kleur boven het veld Kleiner dan wordt gebruikt. De waarden die u in deze velden opgeeft, zijn de aftelbare waarde van de metrische waarde, zoals het aantal paginaweergaven, de dollarbedragen, de tekstweergave, enzovoort. (Gebruik geen speciale tekens.)
   * De Samenvatting van de Reeks van het rapport: Toont geselecteerde metrisch en zijn totaal of hoge en lage waarden voor een rapportreeks.
   * Overzicht van het gebruik: toont gegevens over interfacetoegang door mensen in uw organisatie. Dit rapport kan gegevens door gebruikersnaamtoegang, rapporttoegang, of rapportreekstoegang tonen.
U kunt de volgende rapporten van de Inhoud van de Gebruiker tot stand brengen door URLs te verstrekken. Als een afbeelding of andere bron-URL niet begint met https://, zien Internet Explorer-gebruikers mogelijk een waarschuwing over gemengde inhoud. U kunt de waarschuwing voor gemengde inhoud in uw browser veiligheidsmontages onbruikbaar maken.
   ...
   * HTML: Hiermee kunt u een aangepast HTML-rapport toevoegen. De URL moet HTTP of HTTPS gebruiken. Anders ziet u een `Specified URL could not be retrieved` fout. In de inhoud van het document worden alle tags met kenmerken die de protocollen data: en javascript: gebruiken, verwijderd. Scripts, frames, applets, gebeurtenishandlers, flash en andere ingesloten objecten worden verwijderd. Als bronnen worden opgegeven met behulp van niet-HTTPS, krijgen IE-gebruikers een waarschuwing over gemengde inhoud.
   * Afbeelding: hiermee kunt u een dashboard maken van een afbeeldings-URL. Als de URL het HTTP-protocol gebruikt, geeft Internet Explorer een waarschuwing voor gemengde inhoud weer. Als u een URL gebruikt met HTTPS, wordt de waarschuwing verwijderd. Alle andere protocollen geven a uit `Specified URL could not be retrieved` fout.
...
| Naar | Doe dit |
|--- |--- |
| De rapportsuite van een dashboard wijzigen | Klik het menu in de kopbal van het Experience Cloud, dan selecteer een rapportreeks. |
...
| Een segment toepassen op een dashboard | Klik in de koptekst van het Experience Cloud op [!UICONTROL Show Segments]Selecteer vervolgens een segment. |
...
| Een dashboardarchief weergeven | Klikken  [!UICONTROL More] >  [!UICONTROL View Archive]. |
...
1. Op de [!UICONTROL Company Settings] pagina, klikt u **[!UICONTROL Co-Brand the Adobe Experience Cloud]**.
